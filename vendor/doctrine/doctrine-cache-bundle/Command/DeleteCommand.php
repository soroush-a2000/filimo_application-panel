...
class DeleteCommand extends CacheCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ( ! $all) {
            $message = "حذف کردن از <$color>%s</$color> که در <$color>%s</$color> دارای <$color>%s</$color>";
            $message = sprintf($message, $cacheId, $cacheName, $success, true);
        }

        if ($all) {
            $message = "حذف کردن از <$color>همه</$color> ورودی ها در <$color>%s</$color> دارای <$color>%s</$color>";
